...
let diskPromise = null; // snapshot compartilhado do estado 'options-cache'
...
function diskCache() {
  if (diskPromise === null) diskPromise = loadState(CACHE_KEY, {});
  return diskPromise;
}
...
export async function cachedOptions(key, compute) {
  ...
  mem[key] = entry;
  // Chamadas concorrentes mutam o mesmo objeto `d` (a promise resolve uma vez
  // só), então cada gravação carrega todas as chaves já resolvidas (a última a
  // escrever não apaga as outras).
  d[key] = entry;
  await saveState(CACHE_KEY, d);
  return data;
}
